...
它适合运行在 NAS、Unraid、家庭服务器或 VPS 上。你可以接入多个云盘账号，管理目录与分享，创建订阅任务，配置季集识别、重命名和同集更新规则，让追更、转存、下载和记录归档集中在一个 Web 界面里完成。
...
- 支持配置 Aria2 RPC 地址、密钥、默认下载目录和启用状态。
